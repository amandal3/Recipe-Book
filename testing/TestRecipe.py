...
def optionOne(include, dontInclude, sort, urlS):
    ...
    arrayAns = [len(hp), hp, fridge[1].get('name'),hp.get('Recipe Name')]
    return arrayAns

def optionTwo(keywords, sort, urlS):
    ...
    url = urlS
    hp = allRecipes_Info(url)
    arrayAns = [len(hp), hp, allR[1].get('name'),hp.get('Recipe Name')]
    return arrayAns

def optionThree(keywords, include, dontInclude, sort, urlS):
    ...
    arrayAns = [len(hp), hp, fridgeR[1].get('name'),hp.get('Recipe Name')]
    return arrayAns

def optionFour(keywords, include, dontInclude, sort, number):
    if not sort:
        sort = 're'
    ...
    cS = complexitySearch2(DoR,number)

    if cS == []:
        return [[],len(allRs)]
    arrayAns = [len(cS),len(allRs),cS[0],cS[0].get('Recipe Name'),cS[-1].get('Recipe Name')]
    return arrayAns
